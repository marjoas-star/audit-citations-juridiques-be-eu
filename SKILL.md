...
8. **Ne jamais contourner** paywall, authentification, DRM, CAPTCHA, vérification anti-robot ou restriction technique. Utiliser une autre voie publique que le même site officiel **affiche** sans contrôle (son moteur de recherche public, un lien ou un bouton d'export visible) n'est pas un contournement. En est un : imiter un navigateur, répéter les tentatives, passer par un intermédiaire (proxy, VPN, service tiers) pour masquer l'origine de la connexion, ou atteindre le contenu protégé par une adresse tirée du code du site. Utiliser le navigateur de l'application de bureau, qui consulte le site depuis la connexion de l'utilisateur comme il le ferait lui-même, est la voie normale prévue par ce skill, non un contournement : un site qui refuse les connexions venant de serveurs ne protège pas son contenu contre ses lecteurs. **Face à une vérification anti-robot** (« vérifiez que vous êtes humain », page d'attente de sécurité) : s'arrêter pour cette source, noter l'accès bloqué (`TECHNICAL_FAILURE`), et proposer au juriste de valider lui-même la vérification dans le navigateur de l'application avant de reprendre ; sans lui, poursuivre par une autre source officielle qui publie le même document, ou laisser la référence non vérifiée.
...
**Intégrité** : `FAITHFUL`, `MATERIAL_BUT_NOT_MISLEADING`, `MISLEADING`, `NOT_VERIFIABLE`. Une citation peut être `EXACT_WITH_SIGNALLED_ADAPTATIONS` tout en étant `MISLEADING`. Un changement de mots (substitution, omission, insertion) qui touche un délai ou son point de départ, une condition, une exception, une négation, une obligation ou faculté, ou la personne visée est `MISLEADING` et de gravité `CRITICAL`, sauf à établir sur le texte que la portée est identique ; il n'est jamais « sans déformation » au seul motif que les faits de l'espèce n'en sont pas affectés. `NOT_VERIFIABLE` est réservé au cas où le contexte nécessaire n'a pas pu être lu : si le constat décrit l'effet, l'intégrité est qualifiée.
...
- **Version non applicable** : si les mots cités correspondent exactement à une autre version du texte (antérieure ou future) que celle applicable à la date juridique, ce n'est pas une citation inexacte : la citation est fidèle à cette autre version. Le dire ainsi (« citation exacte d'une version qui n'était pas / plus en vigueur à la date retenue »), indiquer la version applicable et son texte, et ne jamais écrire « sens déformé » ni prêter une altération à l'auteur. Réserver `INEXACT` aux mots qui ne correspondent à aucune version. Dans les données, porter `applicable_version: false` sur la citation : le rapport affiche alors « Texte d'une autre version » et non « Sens préservé ».
- **Traductions** : si une citation semble traduite d'une autre version linguistique, ouvrir cette version et comparer avant de conclure ; ne pas se contenter de le supposer. Une traduction **fidèle** que l'auteur n'a pas signalée (« traduction libre », « vrije vertaling ») donne lieu à une **suggestion** (le signaler ou citer l'original), pas à une erreur, **lorsque la source n'existe pas officiellement dans la langue du document** (par exemple un arrêt rendu dans une seule langue) : elle n'entre pas dans le compte des corrections. Si une version officielle existe dans la langue du document, présenter entre guillemets sa propre traduction comme texte de la source est une erreur (`MINOR`, ou plus si le sens change) : proposer le texte de la version officielle. Une traduction infidèle reste une erreur. Appliquer [language-policy.md](references/language-policy.md). Une comparaison entre langues différentes ne produit jamais `EXACT` : utiliser `NOT_APPLICABLE_TRANSLATION` et évaluer séparément la fidélité (`FAITHFUL`, `FAITHFUL_WITH_MINOR_VARIATION`, `PARTIALLY_FAITHFUL`, `MISLEADING`, `NOT_VERIFIABLE`). Si la version citée est inconnue, ne pas supposer une traduction d'auteur.
...
- **la date juridique retenue** : celle que l'utilisateur a indiquée ; à défaut, celle que le document permet de déduire, présentée comme une hypothèse et avec l'indice qui la fonde (à défaut d'une date précise, au moins une borne : « postérieur au 31 octobre 2025, date de la source la plus récente citée » ; retenir la plus tardive de toutes les dates du document, sources et événements mentionnés compris) ; à défaut, le droit en vigueur au jour de l'audit. Toujours ajouter que l'utilisateur peut indiquer une autre date (« vérifie au 1er janvier 2016 ») ; ne pas attendre sa réponse pour commencer, mais s'il en donne une, refaire les seuls contrôles qui dépendent de la date (versions des textes, décisions postérieures) ;
...
Pour un document très long (plus d'une cinquantaine de sources), proposer de commencer par une partie (un chapitre, les notes d'une section) ou de traiter le document par lots, chaque lot faisant l'objet d'un rapport daté. Pour plusieurs documents, les traiter l'un après l'autre, un rapport par document. Ne pas confier des documents à des agents parallèles qui partagent le même navigateur : ils se gênent (un onglet navigué par un autre agent fausse les recherches).
...
- tout écart aux règles cardinales consigné dans les traces (recherche fondée sur la mémoire, accès bloqué, adresse non affichée) figure aussi dans les limites du rapport ;
