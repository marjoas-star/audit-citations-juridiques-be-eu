...
- adapter la longueur : viser 5 à 6 pages pour une douzaine de sources, avec une synthèse de quelques phrases, des contrôles d'une ligne, des constats de deux phrases et des passages probants limités aux mots qui prouvent (voir [report-rendering.md](templates/report-rendering.md)) ; aucune page distincte imposée par rubrique. Si le générateur signale des champs trop longs ou un rapport au-delà de la cible, raccourcir les champs signalés et relancer **une fois** avant de livrer, sans jamais supprimer une correction, une preuve ou une limite ;
