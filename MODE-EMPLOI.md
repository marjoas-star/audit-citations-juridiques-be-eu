...
- Le fichier du skill, à télécharger en un clic : **[télécharger la dernière version](https://github.com/marjoas-star/audit-citations-juridiques-be-eu/releases/latest/download/audit-citations-juridiques-be-eu.zip)**. Ce lien donne toujours la version la plus récente. Vous pouvez aussi passer par la page [Releases du dépôt](https://github.com/marjoas-star/audit-citations-juridiques-be-eu/releases), rubrique « Assets » (pas le fichier qui se termine par `-complet`, réservé à la maintenance). **Ne décompressez pas l'archive.**
  - **Sur Mac avec Safari**, le fichier est décompressé automatiquement et vous obtenez un dossier. Deux solutions : clic droit sur le dossier › **Compresser « audit-citations-juridiques-be-eu »**, ce qui recrée un `.zip` utilisable ; ou, une fois pour toutes, **Safari › Réglages › Général**, décocher **« Ouvrir les fichiers fiables après téléchargement »**. Chrome et Firefox ne décompressent pas.
...
**Installer une nouvelle version.** Téléchargez la nouvelle archive (même bouton). Dans **Paramètres › Compétences**, ouvrez la fiche du skill, menu **⋮** › **Supprimer**, puis refaites les étapes 3 à 5. N'utilisez pas « Remplacer » : lors de nos essais (septembre 2026), la fiche se dit mise à jour mais garde l'ancien contenu. Pour vérifier la version installée, ouvrez l'onglet **Contenu** de la fiche, fichier `SKILL.md` : la version figure sous le titre (la mention « v1 » de la fiche est un compteur propre à l'application). Elle figure aussi à la fin de chaque rapport.
...
Le skill suit un format ouvert que ChatGPT accepte aussi. **Il n'a pas encore été testé avec ChatGPT** : vos retours sont particulièrement bienvenus.

- **Disponibilité** : selon OpenAI, les skills sont proposés dans ChatGPT **Business, Enterprise, Healthcare et Edu**. L'administrateur de l'espace de travail doit autoriser les skills et leur import (« Enable skills », « Enable skill uploading »).
- **Import** : barre latérale › **Plugins** › onglet **Skills** › **Create** › **Upload from your computer**, puis choisir le même fichier `.zip`. ChatGPT analyse le skill avant de le rendre disponible ; il peut être marqué « Needs Review ».
- **Utilisation** : lancer l'audit dans **ChatGPT Work**, de préférence dans l'**application de bureau** (travail en local, avec le navigateur de votre ordinateur). Sur le web, Work fonctionne sur les serveurs d'OpenAI : EUR-Lex, CURIA et le Conseil d'État risquent alors de refuser les connexions, comme avec Claude dans le cloud.
- La demande-type de la section 5 s'utilise telle quelle.

Aide officielle : [Skills in ChatGPT](https://help.openai.com/en/articles/20001066-skills-in-chatgpt), [ChatGPT Work](https://help.openai.com/en/articles/20001275-chatgpt-work-and-codex).
